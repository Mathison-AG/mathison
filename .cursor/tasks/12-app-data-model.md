...
Transform the Recipe model from a technical Helm chart wrapper into a rich "App Store" listing. Add consumer-facing metadata: short descriptions, use-case tags, getting-started guides, screenshots, and a size-tier abstraction that hides CPU/memory from users. After this step, every recipe has the data needed to render a polished app store card and detail page.
...
model Recipe {
  ...
  // Size tiers (NEW) — abstracts CPU/memory into human-friendly choices
  sizeTiers         Json            @default("{}") @map("size_tiers")
  // Shape: { small: { cpu_request, memory_request, cpu_limit, memory_limit, label },
  //          medium: { ... }, large: { ... } }
...
### 2. Size Tier Type Definition (`src/types/recipe.ts`)

Add the SizeTier interface to the shared recipe types:

```typescript
export interface SizeTier {
  label: string;           // "Small", "Medium", "Large"
  description: string;     // "Good for personal use", "For small teams", "High performance"
  cpu_request: string;     // "100m"
  memory_request: string;  // "256Mi"
  cpu_limit: string;       // "500m"
  memory_limit: string;    // "512Mi"
}
...
### 4. Update Seed Data (`src/lib/catalog/seed-data.ts`)
...
- sizeTiers: small (256Mi), medium (512Mi), large (1Gi)
...
- sizeTiers: small (128Mi), medium (256Mi), large (512Mi)
...
- sizeTiers: small (512Mi), medium (1Gi), large (2Gi)
...
### 5. Update Catalog Service (`src/lib/catalog/service.ts`)
...
### 6. Update Catalog API (`src/app/api/catalog/route.ts`, `[slug]/route.ts`)

Include new fields in API responses:
- List endpoint: add `shortDescription`, `useCases`, `installCount`, `featured`
- Detail endpoint: add all new fields including `gettingStarted`, `screenshots`, `sizeTiers`, `websiteUrl`
...
- [ ] All 5 seed recipes have consumer-facing content (shortDescription, useCases, gettingStarted, sizeTiers)
- [ ] `GET /api/catalog` returns enriched recipe data
- [ ] `GET /api/catalog/[slug]` returns full detail including gettingStarted and sizeTiers
- [ ] SizeTier type is defined and exported from `src/types/recipe.ts`
...
src/
├── types/recipe.ts                  # SizeTier types added
├── lib/catalog/
...
- `sizeTiers` is JSON because different apps have different resource profiles. The type is enforced in TypeScript, not at the DB level.
