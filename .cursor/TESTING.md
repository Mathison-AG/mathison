...
> Verified on 2026-02-13. This is the repeatable testing procedure for the complete Mathison MVP.

## Quick Start: Full E2E Test

Run this checklist at the start of any testing session:
...
This is the critical path that tests the entire stack:
...
Kind clusters can't pull from Docker Hub without internet or with rate limits. Pre-load images:
...
# Pull + load PostgreSQL (for postgres deployments)
docker pull bitnami/postgresql:latest
kind load docker-image bitnami/postgresql:latest --name mathison-dev

# Pull + load MinIO (for minio deployments)
...
| Bitnami image tags         | Pinned chart versions reference image tags that get removed from Docker Hub over time  | Don't pin `chartVersion` for test recipes, or use `NULL` to get latest                  |
| Image pull in kind         | Kind can't pull from Docker Hub easily                                                 | Pre-load images with `kind load docker-image`                                           |
| Card navigation            | Fixed in this session — nested `<a>` tags in DeploymentCard, inline Link in RecipeCard | Links now use `className="block"` and inner links replaced with `<span>` + `onClick`    |
